...
type ProgramM = Reader Program

newtype WellTypingFact = WTFact (Seq Type, Seq Type) deriving (Eq,Ord)

instance Show WellTypingFact where
...
wtBot :: WellTypingFact
wtBot = WTFact (Seq.empty,Seq.empty)

wtJoin :: Q -> WellTypingFact -> WellTypingFact -> WellTypingFact
wtJoin q =  wtJoin' (qprogram q)
  where
