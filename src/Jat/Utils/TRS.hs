...
prettyPARule (r,Nothing) = prettyRule r
prettyPARule (r,Just c) = prettyRule r <+> text "<==" <+> PA.prettyPATerm c

type PARule = (R.Rule PA.PAFun PA.PAVar, Maybe PA.PATerm)
...
header rules =
  ...
  <$> lparen <> text "RULES"
  where
    prettyvars = const empty
    allvars    = foldl (\s -> S.union s . S.fromList . rulevars) S.empty
    rulevars (r,c) = R.vars r ++ maybe [] T.vars c
...
prettyTRS :: [(R.Rule PA.PAFun PA.PAVar, Maybe PA.PATerm)] -> Doc
prettyTRS crules =
  header crules
